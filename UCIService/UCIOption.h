...
template <class T> class UCIOption {
  public:
    using value_type = T;
    ...
    void handler(std::optional<value_type> value) const noexcept { return handler_(value); }
...
template <> class UCIOption<void> {
  public:
    ...
    void handler() const noexcept { handler_(); }
...
class UCISpinOption : public UCIOption<int> {
  public:
    ...
    UCISpinOption(const std::string& name, const value_type& value, const value_type& min_value,
                  const value_type& max_value,
                  const std::function<void(const value_type&)>& handler) noexcept
        : UCIOption<value_type>(name, value, handler), min_value_(min_value),
          max_value_(max_value) {}

    [[nodiscard]] value_type min_value() const noexcept { return min_value_; }
    [[nodiscard]] value_type max_value() const noexcept { return max_value_; }
...
class UCIComboOption : public UCIOption<std::string> {
  public:
    ...
    UCIComboOption(const std::string& name, const value_type& value,
...
class UCIStringOption : public UCIOption<std::string> {
  public:
    ...
    UCIStringOption(const std::string& name, const value_type& value,
...
class UCICheckOption : public UCIOption<bool> {
  public:
    ...
    UCICheckOption(const std::string& name, const value_type& value,
                   const std::function<void(const value_type&)>& handler)
        : UCIOption<value_type>(name, value, handler) {}
};
