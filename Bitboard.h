...
class Bitboard {
  public:
    ...
    constexpr Bitboard operator<<(int shift) const { return Bitboard{value_ << unsigned(shift)}; }
    constexpr Bitboard operator<<(unsigned shift) const { return Bitboard{value_ << shift}; }
    constexpr Bitboard operator>>(int shift) const { return Bitboard{value_ >> unsigned(shift)}; }
    constexpr Bitboard operator>>(unsigned shift) const { return Bitboard{value_ >> shift}; }
    constexpr Bitboard operator|(value_type rhs) const { return Bitboard{value_ | rhs}; }
    constexpr Bitboard operator&(value_type rhs) const { return Bitboard{value_ & rhs}; }
    constexpr Bitboard operator^(value_type rhs) const { return Bitboard{value_ ^ rhs}; }
    constexpr Bitboard operator|(Bitboard rhs) const { return Bitboard{value_ | rhs}; }
    constexpr Bitboard operator&(Bitboard rhs) const { return Bitboard{value_ & rhs}; }
    constexpr Bitboard operator^(Bitboard rhs) const { return Bitboard{value_ ^ rhs}; }

    constexpr Bitboard operator~() const { return Bitboard{~value_}; }
    ...
    constexpr int popcount() const { return __builtin_popcountll(value_); }
    constexpr Square forward_bitscan() const { return Square{__builtin_ctzll(value_)}; }
    constexpr Square reverse_bitscan() const { return Square{63 - __builtin_clzll(value_)}; }
    constexpr void forward_popbit() { value_ &= value_ - 1; }
    constexpr void reverse_popbit() { value_ ^= Bitboard{reverse_bitscan() + 1}; }
